...
namespace ZLinq.Tests
{
    public class SelectTests : EnumerableTests
    {
        ...
        [Fact] // [ConditionalFact(typeof(PlatformDetection), nameof(PlatformDetection.IsThreadingSupported))]
        ...
        public void Select_SourceIsIList_ElementAt()
        {
            ...
            for (int i = 0; i != 4; ++i)
                Assert.Equal(i * 2 + 2, source.ElementAt(i));
            AssertExtensions.Throws<ArgumentOutOfRangeException>("index", () => new List<int> { 1, 2, 3, 4 }.AsReadOnly().Select(i => i * 2).ElementAt(-1));
            AssertExtensions.Throws<ArgumentOutOfRangeException>("index", () => new List<int> { 1, 2, 3, 4 }.AsReadOnly().Select(i => i * 2).ElementAt(4));
            AssertExtensions.Throws<ArgumentOutOfRangeException>("index", () => new List<int> { 1, 2, 3, 4 }.AsReadOnly().Select(i => i * 2).ElementAt(40));

            Assert.Equal(6, source.Skip(1).ElementAt(1));
            AssertExtensions.Throws<ArgumentOutOfRangeException>("index", () => new List<int> { 1, 2, 3, 4 }.AsReadOnly().Select(i => i * 2).Skip(2).ElementAt(9));
        }
        ...
        public void MoveNextAfterDispose(IEnumerable<int> source)
        {
            ...
            {
                IEnumerable<int> result = equivalentSource.Select(i => i).ToArray();
        ...
        public static IEnumerable<object[]> RunSelectorDuringCountData()
        {
            ...
            for (int i = 0; i <= 5; i++)
            {
                ...
                {
                    yield return [transform(enumerable.ToArray())];
                }
            }
        }
    }
