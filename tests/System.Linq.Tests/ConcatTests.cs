...
namespace ZLinq.Tests
{
    public class ConcatTests : EnumerableTests
    {
        ...
        {
            first = (from item in first select item).ToArray();
            second = (from item in second select item).ToArray();
        ...
        public void PossiblyEmptyInputs(IEnumerable<int> first, IEnumerable<int> second, IEnumerable<int> expected)
        {
            ...
            Assert.Equal(expected.Skip(first.Count()).Concat(expected.Take(first.Count())), second.Concat(first).ToArray()); // Swap the inputs around
        }
        ...
        {
            var source = Enumerable.Repeat(1, 4).Concat(Enumerable.Repeat(1, 5)).ToArray();
            ...
            for (int i = 0; i < (1 << EnumerableCount); i++)
            {
                ...
                for (int j = 0; j < (1 << EnumerableCount); j++)
                {
                    for (int k = 0; k < EnumerableCount; k++) // k is how much bits we shift by, and also the item that gets appended/prepended.
                    {
                        ...
                        actual = prepend ? nextRange.Concat(actual).ToArray() : actual.Concat(nextRange).ToArray();
                        if (prepend)
                        {
                            expected.Insert(0, k);
                        }
        ...
        public void ManyConcats(IEnumerable<IEnumerable<int>> sources)
        {
            foreach (var transform in IdentityTransforms<int>())
            {
                ...
                {
                    concatee = concatee.Concat(transform(source)).ToArray();
                }
