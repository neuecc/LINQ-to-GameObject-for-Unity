...
namespace ZLinq.Tests
{
    public class CountTests : EnumerableTests
    {
        ...
        public static IEnumerable<object[]> NonEnumeratedCount_SupportedEnumerables()
        {
            ...
            {
                yield return WrapArgs(50, Enumerable.Range(1, 50).Select(x => x + 1).ToArray());
                yield return WrapArgs(4, new int[] { 1, 2, 3, 4 }.Select(x => x + 1).ToArray());
                yield return WrapArgs(50, Enumerable.Range(1, 50).Select(x => x + 1).Select(x => x - 1).ToArray());
                yield return WrapArgs(20, Enumerable.Range(1, 20).Reverse().ToArray());
                yield return WrapArgs(20, Enumerable.Range(1, 20).OrderBy(x => -x).ToArray());
                yield return WrapArgs(20, Enumerable.Range(1, 10).Concat(Enumerable.Range(11, 10)).ToArray());
            }
        ...
        {
            yield return WrapArgs(Enumerable.Range(1, 100).Where(x => x % 2 == 0).ToArray());
            yield return WrapArgs(Enumerable.Range(1, 100).GroupBy(x => x % 2 == 0).ToArray());
            yield return WrapArgs(new Stack<int>([1, 2, 3, 4]).Select(x => x + 1).ToArray());
            yield return WrapArgs(Enumerable.Range(1, 100).Distinct().ToArray());

            // if (!PlatformDetection.IsLinqSpeedOptimized)
            {
                yield return WrapArgs(Enumerable.Range(1, 50).Select(x => x + 1).ToArray());
                yield return WrapArgs(new int[] { 1, 2, 3, 4 }.Select(x => x + 1).ToArray());            
                yield return WrapArgs(Enumerable.Range(1, 50).Select(x => x + 1).Select(x => x - 1).ToArray());
                yield return WrapArgs(Enumerable.Range(1, 20).Reverse().ToArray());
                yield return WrapArgs(Enumerable.Range(1, 20).OrderBy(x => -x).ToArray());
                yield return WrapArgs(Enumerable.Range(1, 10).Concat(Enumerable.Range(11, 10)).ToArray());
            }
