    ...
    public class ContainsTests : EnumerableTests
    {
        ...
        public void FollowingVariousOperators()
        {
            ...
            foreach (var transform in IdentityTransforms<int>())
            {
                ...
                // Distinct
                Assert.True(transform(source.Concat(source).ToArray()).Distinct().Contains(2));
                Assert.False(transform(source.Concat(source).ToArray()).Distinct().Contains(4));
                Assert.True(transform(source.Concat(source).ToArray()).Distinct().Contains(1));
                Assert.True(transform(source.Concat(source).ToArray()).Distinct(EqualityComparer<int>.Create((x, y) => true, x => 0)).Contains(1));
                Assert.False(transform(source.Concat(source).ToArray()).Distinct(EqualityComparer<int>.Create((x, y) => true, x => 0)).Contains(2));
                Assert.False(transform(source.Concat(source).ToArray()).Distinct(EqualityComparer<int>.Create((x, y) => true, x => 0)).Contains(0));
