    ...
    public class ToDictionaryTests : EnumerableTests
    {
        ...
        public void RunOnce()
        {
            ...
            Assert.Equal(
                ...
                Enumerable.Range(0, 4).Select(i => new KeyValuePair<int, string>(i, i.ToString())).ToArray().RunOnce().ToDictionary());

            Assert.Equal(
                new Dictionary<int, string> { { 0, "0" }, { 1, "1" }, { 2, "2" }, { 3, "3" } },
                Enumerable.Range(0, 4).Select(i => (i, i.ToString())).ToArray().RunOnce().ToDictionary());
        }
