...
namespace ZLinq.Tests
{
    public class AnyTests : EnumerableTests
    {
        ...
        public static IEnumerable<object[]> TestData()
        {
            foreach (int count in new[] { 0, 1, 2 })
            {
                ...
                foreach (IEnumerable<int> source in CreateSources(new int[count]))
                {
                    ...
                    yield return [source.Select(i => i).ToArray(), expected];
                    yield return [source.Where(i => true).ToArray(), expected];

                    yield return [source.Where(i => false).ToArray(), false];
                }
            }
        }
        ...
        {
            yield return [Array.Empty<int>().GroupBy(num => num).ToArray(), false];
            yield return [new int[2] { 1, 2 }.GroupBy(num => num).ToArray(), true];
            yield return [new int[5] { 1, 2, 1, 3, 2 }.GroupBy(n => n, (k, v) => v).ToArray(), true];
        }
