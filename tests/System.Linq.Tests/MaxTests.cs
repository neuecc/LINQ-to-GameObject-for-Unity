...
namespace ZLinq.Tests
{
    public class MaxTests : EnumerableTests
    {
        public static IEnumerable<object[]> Max_AllTypes_TestData()
        {
            ...
            {
                yield return [Enumerable.Range(length, length).Select(i => (byte)i).Shuffle().ToArray(), (byte)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (byte)i).ToArray().Shuffle().ToArray(), (byte)(length + length - 1)];

                // Unit Tests does +T.One so we should generate data up to one value below sbyte.MaxValue
                if ((length + length) < sbyte.MaxValue) {
                    yield return [Enumerable.Range(length, length).Select(i => (sbyte)i).Shuffle().ToArray(), (sbyte)(length + length - 1)];
                    yield return [Enumerable.Range(length, length).Select(i => (sbyte)i).ToArray().Shuffle().ToArray(), (sbyte)(length + length - 1)];
                }

                yield return [Enumerable.Range(length, length).Select(i => (ushort)i).Shuffle().ToArray(), (ushort)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (short)i).Shuffle().ToArray(), (short)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (char)i).Shuffle().ToArray(), (char)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (uint)i).Shuffle().ToArray(), (uint)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (int)i).Shuffle().ToArray(), (int)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (ulong)i).Shuffle().ToArray(), (ulong)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (long)i).Shuffle().ToArray(), (long)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (float)i).Shuffle().ToArray(), (float)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (double)i).Shuffle().ToArray(), (double)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (decimal)i).Shuffle().ToArray(), (decimal)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (nuint)i).Shuffle().ToArray(), (nuint)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (nint)i).Shuffle().ToArray(), (nint)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (Int128)i).Shuffle().ToArray(), (Int128)(length + length - 1)];
                yield return [Enumerable.Range(length, length).Select(i => (UInt128)i).Shuffle().ToArray(), (UInt128)(length + length - 1)];
            }
        }
