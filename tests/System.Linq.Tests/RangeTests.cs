...
namespace ZLinq.Tests
{
    public class RangeTests : EnumerableTests
    {
        ...
        public void IListImplementationIsValid()
        {
            ...
            Validate(Enumerable.Range(42, 10).Skip(3).Take(4).ToArray(), [45, 46, 47, 48]);

            static void Validate(IEnumerable<int> e, int[] expected)
            {
                IList<int> list = Xunit.Assert.IsAssignableFrom<IList<int>>(e);
                IReadOnlyList<int> roList = Xunit.Assert.IsAssignableFrom<IReadOnlyList<int>>(e);
