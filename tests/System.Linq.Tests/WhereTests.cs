...
namespace ZLinq.Tests
{
    public class WhereTests : EnumerableTests
    {
        ...
        public void WhereWhere_IEnumerable_ExecutionIsDeferred()
        {
            ...
            var query2 = query.Where((value, index) => value());
            Assert.False(funcCalled);
        }
        ...
        {
            int[] source = [1, 2, 3, 4, 5];
            using var enumerator = source.Where(value => true).GetEnumerator();
        ...
        public void SameResultsRepeatCallsIntQuery()
        {
            ...
            Assert.Equal(q.Where(IsEven).ToArray(), q.Where(IsEven));
        ...
        public void SameResultsRepeatCallsStringQuery()
        {
            ...
            Assert.Equal(q.Where(string.IsNullOrEmpty).ToArray(), q.Where(string.IsNullOrEmpty));
        ...
        public void PredicateTrueFirstOnly()
        {
            ...
            Assert.Equal(source.Take(1).ToArray(), source.Where(IsEven));
        }
        ...
        public void PredicateTrueLastOnly()
        {
            ...
            Assert.Equal(source.Skip(source.Length - 1).ToArray(), source.Where(IsEven));
        }
        ...
        {
            int[] source = [10, 9, 7, 15, 3, 27];
            Assert.Equal(source.Take(1), source.Where((e, i) => e % 2 == 0).ToArray());
        }
        ...
        {
            int[] source = [9, 7, 15, 3, 27, 20];
            Assert.Equal(source.Skip(source.Length - 1), source.Where((e, i) => e % 2 == 0).ToArray());
        }
        ...
        public void PredicateSelectsFirst()
        {
            ...
            Assert.Equal(source.Take(1), source.Where((e, i) => i == 0).ToArray());
        }
        ...
        {
            int[] source = [-40, 20, 100, 5, 4, 9];
            Assert.Equal(source.Skip(source.Length - 1), source.Where((e, i) => i == source.Length - 1).ToArray());
        }
        ...
        {
            foreach (IEnumerable<int> equivalent in new[] { source.Where(s => true).ToArray(), source.Where(s => true).Select(s => s).ToArray() })
            {
