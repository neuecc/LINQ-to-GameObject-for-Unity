...
namespace ZLinq.Tests
{
    public class OrderedSubsetting : EnumerableTests
    {
        ...
        {
            Assert.Empty(Enumerable.Range(0, 10).Shuffle().OrderBy(i => i).Skip(42).ToArray());
        }
        ...
        [Fact] // [ConditionalFact(typeof(PlatformDetection), nameof(PlatformDetection.IsLinqSpeedOptimized))]
        ...
        public void TakeThenTakeExcessive()
        {
            var source = Enumerable.Range(0, 100).Shuffle().ToArray();
            var ordered = source.OrderBy(i => i);
            Assert.Equal(ordered.Take(20), ordered.Take(20).Take(100).ToArray());
        }
        ...
        {
            Assert.Equal([1], Enumerable.Range(1, 10).Shuffle().OrderBy(i => i).Take(1).ToArray());
            Assert.Equal([2], Enumerable.Range(1, 10).Shuffle().OrderBy(i => i).Skip(1).Take(1).ToArray());
            Assert.Equal([3], Enumerable.Range(1, 10).Shuffle().OrderBy(i => i).Take(3).Skip(2).ToArray());
            Assert.Equal([1], Enumerable.Range(1, 10).Shuffle().OrderBy(i => i).Take(3).Take(1).ToArray());
        }
        ...
        public void Select()
        {
            Assert.Equal([0, 2, 4, 6, 8], Enumerable.Range(-1, 8).Shuffle().OrderBy(i => i).Skip(1).Take(5).Select(i => i * 2).ToArray());
        }
        ...
        public void SelectElementAt()
        {
            ...
            AssertExtensions.Throws<ArgumentOutOfRangeException>("index", () => Enumerable.Range(0, 9).Shuffle().OrderBy(i => i).Skip(1).Take(5).Select(i => i * 2).ElementAt(-2));
        }
        ...
        public void SelectFirst()
        {
            ...
            var source2 = source.Skip(20);
            Assert.Equal(0, source2.FirstOrDefault());
        ...
        public void SelectLast()
        {
            ...
            var source2 = source.Skip(20);
            Assert.Equal(0, source2.LastOrDefault());
