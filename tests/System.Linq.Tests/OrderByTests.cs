...
namespace ZLinq.Tests
{
    public class OrderByTests : EnumerableTests
    {
        ...
        public void FirstWithPredicateOnOrdered()
        {
            ...
            counter = 0;
            Assert.Throws<InvalidOperationException>(() => Enumerable.Range(0, 10).Shuffle().OrderBy(i => i).First(i => { counter++; return false; }));
            Assert.Equal(10, counter);
            ...
            counter = 0;
            Assert.Throws<InvalidOperationException>(() => Enumerable.Range(0, 10).Shuffle().OrderByDescending(i => i).First(i => { counter++; return false; }));
            Assert.Equal(10, counter);
        }
        ...
        public void OrderByIsCovariantTestWithCast()
        {
            ...
            var covariantOrdered2 = covariantOrdered.ThenBy(i => i);
            string[] expected =
                Enumerable.Range(0, 100).Select(i => i.ToString()).OrderBy(i => i.Length).ThenBy(i => i).ToArray();
            Assert.Equal(expected, covariantOrdered2);
        }
        ...
        public void CanObtainFromCovariantIOrderedQueryable()
        {
            ...
            var ordered2 = ordered.ThenBy(i => i);
            string[] expected =
                Enumerable.Range(0, 100).Select(i => i.ToString()).OrderBy(i => i.Length).ThenBy(i => i).ToArray();
            Assert.Equal(expected, ordered2);
        }
        ...
        public void CultureOrderBy()
        {
            ...
            using (new ThreadCultureChange(dk))
            {
                ...
                Xunit.Assert.Equal(resultDK, check, StringComparer.Ordinal);
            }

            using (new ThreadCultureChange(au))
            {
                check = source.OrderBy(x => x).ToArray();
                Xunit.Assert.Equal(resultAU, check, StringComparer.Ordinal);
            }
            ...
            using (new ThreadCultureChange(dk)) // "dk" whilst GetEnumerator
            {
                ...
                using (new ThreadCultureChange(au)) // but "au" whilst accessing...
                {
                    ...
                    {
                        Xunit.Assert.Equal(resultAU[idx++], s.Current, StringComparer.Ordinal);
                    }
                }
            }
            ...
            using (new ThreadCultureChange(au))
            {
                ...
                using (new ThreadCultureChange(dk))
                {
                    ...
                    using (new ThreadCultureChange(au)) // "au" whilst GetEnumerator
                    {
                        ...
                        {
                            Xunit.Assert.Equal(resultDK[idx++], s.Current, StringComparer.Ordinal);
                            moveNext = s.MoveNext();
                        }
                    }
                }
        ...
        public void CultureOrderByElementAt()
        {
            ...
            for (int i = 0; i < source.Length; ++i)
            {
                using (new ThreadCultureChange(dk))
                {
                    Xunit.Assert.Equal(resultDK[i], delaySortedSource.ElementAt(i), StringComparer.Ordinal);
                }

                using (new ThreadCultureChange(au))
                {
                    Xunit.Assert.Equal(resultAU[i], delaySortedSource.ElementAt(i), StringComparer.Ordinal);
                }
            }
        }
