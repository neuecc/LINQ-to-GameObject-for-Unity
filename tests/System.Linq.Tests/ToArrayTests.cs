    ...
    public class ToArrayTests : EnumerableTests
    {
        ...
        public void RunOnce()
        {
            ...
            Assert.Equal(
                ...
                Enumerable.Range(1, 8).Select(i => i.ToString()).ToArray().RunOnce().ToArray());
        }
        ...
        public void ToArray_FailOnExtremelyLargeCollection()
        {
            ...
            var thrownException = Xunit.Assert.ThrowsAny<Exception>(() => { largeSeq.ToArray(); });
            Assert.True(
        ...
        public void ToArray_Cast()
        {
            ...
            var cast = source.AsValueEnumerable().Cast<Enum1>().ToArray();
            Xunit.Assert.IsType<Enum0[]>(cast);
            var castArray = cast.ToArray();
            Xunit.Assert.IsType<Enum1[]>(castArray);
