...
public abstract class EnumerableTests
{
    ...
    {
        var listPartition = source.Skip(0).ToArray();
    ...
    protected static IEnumerable<T> EnumerablePartitionOrEmpty<T>(IEnumerable<T> source) // Or Empty
    {
        ...
        return enumerablePartition.ToArray(); ;
    }
    ...
    protected static IEnumerable<Func<IEnumerable<T>, IEnumerable<T>>> IdentityTransforms<T>()
    {
        ...
        [
            ...
            e =>
                {
                    T[] values = e.ToArray();
                    return values.Length == 0 ? [] : values[0..^1].Append(values[^1]).ToList();
                },

                // Concat
                e => e.Concat(ForceNotCollection<T>([])).ToArray(),
                e => ForceNotCollection<T>([]).Concat(e).ToArray(),

                // Prepend
                e =>
                {
                    T[] values = e.ToArray();
                    return values.Length == 0 ? [] : values[1..].Prepend(values[0]).ToList();
                },

                // Reverse
                e => e.Reverse().Reverse().ToList(),

                // Select
                e => e.Select(i => i).ToList(),

                // SelectMany
                //e => e.SelectMany<T, T>(i => [i]),

                // Take
                e => e.Take(int.MaxValue).ToArray(),
                e => e.TakeLast(int.MaxValue).ToArray(),
                e => e.TakeWhile(i => true).ToArray(),

                // Skip
                e => e.SkipWhile(i => false).ToArray(),

                // Where
                e => e.Where(i => true).ToArray(),
            ];
