...
namespace System
{
    public static class AssertExtensions
    {
        ...
        public static void Throws<T>(Action action, string expectedMessage)
            where T : Exception
        {
            Assert.Equal(expectedMessage, Xunit.Assert.Throws<T>(action).Message);
        }

        public static void ThrowsContains<T>(Action action, string expectedMessageContent)
            where T : Exception
        {
            Xunit.Assert.Contains(expectedMessageContent, Xunit.Assert.Throws<T>(action).Message);
        }

        public static T Throws<T>(string netCoreParamName, string netFxParamName, Action action)
            where T : ArgumentException
        {
            T exception = Xunit.Assert.Throws<T>(action);
            ...
            Xunit.Assert.Equal(expectedParamName, exception.ParamName);
            return exception;
        }
        ...
        public static void Throws<T>(string netCoreParamName, string netFxParamName, Func<object> testCode)
            where T : ArgumentException
        {
            T exception = Xunit.Assert.Throws<T>(testCode);
        ...
        public static async Task<T> ThrowsAsync<T>(string expectedParamName, Func<Task> testCode)
            where T : ArgumentException
        {
            T exception = await Xunit.Assert.ThrowsAsync<T>(testCode);
            ...
            where TNetFxExceptionType : Exception
        {
            if (IsNetFramework)
            {
                ...
                {
                    Exception exception = Xunit.Assert.Throws(typeof(TNetFxExceptionType), action);
        ...
        public static Exception Throws(Type netCoreExceptionType, Type netFxExceptionType, Action action)
        {
            if (IsNetFramework)
            {
                return Xunit.Assert.Throws(netFxExceptionType, action);
            }
            else
            {
                return Xunit.Assert.Throws(netCoreExceptionType, action);
            }
        }
        ...
        {
            OperationCanceledException oce = Xunit.Assert.ThrowsAny<OperationCanceledException>(testCode);
        ...
        {
            OperationCanceledException oce = await Xunit.Assert.ThrowsAnyAsync<OperationCanceledException>(testCode);
            if (cancellationToken.CanBeCanceled)
            {
                Assert.Equal(cancellationToken, oce.CancellationToken);
            }
        ...
        public static void Contains(string value, string substring)
        {
            ...
            Xunit.Assert.NotNull(substring);
            Xunit.Assert.Contains(substring, value, StringComparison.Ordinal);
        }
