...
namespace ZLinq.Tests
{
    public class EmptyEnumerableTest : EnumerableTests
    {
        ...
        {
            var enumerable1 = ValueEnumerable.Empty<T>();
            var enumerable2 = ValueEnumerable.Empty<T>();
        ...
        {
            Assert.Equal(new T[0], ValueEnumerable.Empty<T>());
            Assert.Equal(0, ValueEnumerable.Empty<T>().Count());
            Assert.Same(ValueEnumerable.Empty<T>().GetEnumerator(), ValueEnumerable.Empty<T>().GetEnumerator());
        }
        ...
        //{
        //    IList<int> list = Assert.IsAssignableFrom<IList<int>>(ValueEnumerable.Empty<int>());
        //    IReadOnlyList<int> roList = Assert.IsAssignableFrom<IReadOnlyList<int>>(ValueEnumerable.Empty<int>());
