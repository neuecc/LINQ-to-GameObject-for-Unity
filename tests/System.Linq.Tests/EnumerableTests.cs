    ...
    public abstract class EnumerableTests
    {
        ...
        protected static IEnumerable<Func<IEnumerable<T>, IEnumerable<T>>> IdentityTransforms<T>()
        {
            ...
            [
                ...
                {
                    T[] values = e.ToArray();
                    return values.Length == 0 ? [] : values[0..^1].Append(values[^1]).ToArray();
                },
                ...
                {
                    T[] values = e.ToArray();
                    return values.Length == 0 ? [] : values[1..].Prepend(values[0]).ToArray();
                },

                // Reverse
                e => e.Reverse().Reverse().ToArray(),

                // Select
                e => e.Select(i => i).ToArray(),

                // SelectMany
                e => e.SelectMany<T, T>(i => [i]).ToArray(),
