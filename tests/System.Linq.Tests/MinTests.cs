...
namespace ZLinq.Tests
{
    public class MinTests : EnumerableTests
    {
        public static IEnumerable<object[]> Min_AllTypes_TestData()
        {
            ...
            {
                yield return [Enumerable.Range(length, length).Select(i => (byte)i).Shuffle().ToArray(), (byte)length];
                ...
                if ((length + length) < sbyte.MaxValue) {
                    yield return [Enumerable.Range(length, length).Select(i => (sbyte)i).Shuffle().ToArray(), (sbyte)length];
                }

                yield return [Enumerable.Range(length, length).Select(i => (ushort)i).Shuffle().ToArray(), (ushort)length];
                yield return [Enumerable.Range(length, length).Select(i => (short)i).Shuffle().ToArray(), (short)length];
                yield return [Enumerable.Range(length, length).Select(i => (uint)i).Shuffle().ToArray(), (uint)length];
                yield return [Enumerable.Range(length, length).Select(i => (int)i).Shuffle().ToArray(), (int)length];
                yield return [Enumerable.Range(length, length).Select(i => (ulong)i).Shuffle().ToArray(), (ulong)length];
                yield return [Enumerable.Range(length, length).Select(i => (long)i).Shuffle().ToArray(), (long)length];
                yield return [Enumerable.Range(length, length).Select(i => (float)i).Shuffle().ToArray(), (float)length];
                yield return [Enumerable.Range(length, length).Select(i => (double)i).Shuffle().ToArray(), (double)length];
                yield return [Enumerable.Range(length, length).Select(i => (decimal)i).Shuffle().ToArray(), (decimal)length];
                yield return [Enumerable.Range(length, length).Select(i => (nuint)i).Shuffle().ToArray(), (nuint)length];
                yield return [Enumerable.Range(length, length).Select(i => (nint)i).Shuffle().ToArray(), (nint)length];
            }
        }
        ...
        public static IEnumerable<object[]> Min_DateTime_TestData()
        {
            ...
            yield return [hundred.Concat(Enumerable.Repeat(DateTime.MinValue, 1)).ToArray(), DateTime.MinValue];
        }
