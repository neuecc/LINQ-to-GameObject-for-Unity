...
namespace ZLinq.Tests
{
    public partial class GroupByTests : EnumerableTests
    {
        ...
        private static void AssertGroupingCorrect<TKey, TElement>(IEnumerable<TKey> keys, IEnumerable<TElement> elements, IEnumerable<IGrouping<TKey, TElement>> grouping, IEqualityComparer<TKey> keyComparer)
        {
            ...
            {
                Xunit.Assert.NotEmpty(group.ToArray());
                TKey key = group.Key;
                List<TElement> list;
        ...
        public void SameResultsRepeatCallsStringQuery()
        {
            ...
            Assert.Equal(q.GroupBy(e => e.a1, e => e.a2).ToArray(), q.GroupBy(e => e.a1, e => e.a2));
        }
        ...
        public void SingleNullKeySingleNullElement()
        {
            ...
            AssertGroupingCorrect(key, element.ToArray(), new string[] { null }.GroupBy(e => e, e => e, EqualityComparer<string>.Default).ToArray(), EqualityComparer<string>.Default);
        }
        ...
        [Fact] // [ConditionalFact(typeof(PlatformDetection), nameof(PlatformDetection.IsNotBuiltWithAggressiveTrimming))]
        ...
        {
            foreach (var e1 in new[] { Enumerable.Range(0, 10).GroupBy(i => i).ToArray(), Enumerable.Range(0, 10).GroupBy(i => i, i => i).ToArray() })
            {
            ...
            foreach (var e2 in new[] { Enumerable.Range(0, 10).GroupBy(i => i, (i, e) => i).ToArray(), Enumerable.Range(0, 10).GroupBy(i => i, i => i, (i, e) => i).ToArray() })
            {
                for (int trial = 0; trial < 3; trial++)
                {
                    int count = 0;
