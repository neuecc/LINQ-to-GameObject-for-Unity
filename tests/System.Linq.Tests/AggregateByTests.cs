...
namespace ZLinq.Tests
{
    public class AggregateByTests : EnumerableTests
    {
        ...
        public void Empty()
        {
            ...
            {
                Assert.Equal([], source.AggregateBy(i => i, i => i, (a, i) => a + i).ToArray());
                Assert.Equal([], source.AggregateBy(i => i, 0, (a, i) => a + i).ToArray());
            });
        }
        ...
        public void GroupBy()
        {
            static IEnumerable<KeyValuePair<TKey, List<TSource>>> GroupBy<TSource, TKey>(IEnumerable<TSource> source, Func<TSource, TKey> keySelector) =>
                source.AggregateBy(
                    ...
                    (group, element) => { group.Add(element); return group; }).ToArray();
